...
void main(){
    ...
    //RECORDAR LIBERAR MEMORIA, tablero, palabras, nombres archivos, datos de las palabras
    for(int k=0; k<filasM; k++){
        free(tablero[k]);
    }
    free(tablero);
    for(int k=0; k<cantPalabrasArray; k++){
        free(palabrasTablero[k]);
        free(datosPalabras[k].fila);
        free(datosPalabras[k].columna);
    }
    free(palabrasTablero);
    free(datosPalabras);
    for(int k=0; k<12; k++){
        free(nombreLista[k]);
        free(nombreTablero[k]);
    }
    free(nombreLista);
    free(nombreTablero);
