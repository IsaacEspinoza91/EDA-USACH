...
Salida: TDAlistaChar2D (lista de lista de char de las letras de las palabras)*/
...
bool buscarPalabras(TDAlistaChar2D tablero, TDAlistaChar palabra, int mFila, int nColumna, TDAlistaCoordenadas* listCoordenadas , FILE* archivo){
            ...
            if(aux2->dato == palabra->dato){
                ...
                if((columna+largoPalabra-1) < nColumna){
                    ...
                    if(palabraEncontrada == true){ 
                        ...
                        //proceso de escribri la palabra donde estabanlos ****
                        auxPalabra = palabra;
                        numAux = 0;
                        while(auxPalabra != NULL){
    ...
    insertarNodoFinalListaCoordenadas(listCoordenadas, -1, -1); //las coordenadas son -1, =1 cuando no se encuentra la palabra
    return palabraEncontrada;
